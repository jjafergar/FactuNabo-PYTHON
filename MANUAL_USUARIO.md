...
- ✅ Validar facturas antes del envío (incluyendo validación de NIF/CIF/NIE)
...
- ✅ Asistente de rectificativas con detección automática de cambios
- ✅ Rectificar facturas de años anteriores
- ✅ Previsualización mejorada con conceptos
...
2. Revisa que los datos sean correctos:
   ...
   - Clientes
   - Importes
   - **Validación NIF**: La columna "Validación NIF" muestra si el NIF/CIF/NIE del cliente es correcto
     - ✅ **NIF Correcto** (verde): El documento es válido
     - ❌ **NIF Incorrecto** (rojo): El documento tiene errores (pasa el ratón para ver el detalle)
3. Si hay errores, se mostrarán en rojo. Corrígelos en el Excel y vuelve a cargar
4. **Importante**: Si hay errores de validación de NIF, el sistema te advertirá antes de enviar, pero podrás continuar si lo deseas

### Paso 3: Enviar Facturas

1. Ve a **"🚀 Enviar Facturas"** en el menú lateral
2. Verás una tabla de previsualización con las facturas a enviar que incluye:
   - **Factura**: Número de factura (o número de rectificativa si aplica)
   - **Fecha**: Fecha de emisión (centrada y compacta)
   - **Empresa Emisora**: Nombre de la empresa que emite
   - **Cliente**: Nombre del cliente
   - **Concepto**: Primer concepto de la factura (truncado si es muy largo)
   - **Importe Total**: Importe total de la factura
3. Revisa la información mostrada
4. Si hay errores de validación de NIF, el sistema te mostrará una advertencia antes de continuar
5. Haz clic en **"🚀 Iniciar Envío"**
6. El sistema mostrará el progreso:
   - **Paso 1**: Cargar Excel
   - **Paso 2**: Validar
   - **Paso 3**: Listo
...
1. Una vez finalizado, verás los resultados en una tabla que incluye:
   - **Factura**: Número asignado por Facturantia (o número original si hay error)
   - **Empresa Emisora**: Nombre de la empresa
   - **Cliente**: Nombre del cliente
   - **Concepto**: Primer concepto de la factura
   ...
   - **Detalles**: Información adicional sobre el envío
   - **Ver Factura**: Botón para abrir el PDF de la factura
...
4. Puedes exportar los resultados a CSV usando el botón de exportar
...
### ❌ Error: "NIF Incorrecto" en la columna de validación

**Causa**: El NIF/CIF/NIE del cliente no es válido según los algoritmos de validación españoles.

**Solución**:
1. **Para facturas nacionales**: Verifica que el NIF/CIF/NIE esté correctamente escrito
   - NIF: 8 dígitos + 1 letra (ej: 12345678A)
   - CIF: Letra + 7 dígitos + 1 carácter (ej: B12345678)
   - NIE: X/Y/Z + 7 dígitos + 1 letra (ej: X1234567L)
2. **Para facturas intracomunitarias**: El NIF-IVA debe empezar con el código de país de la UE (ej: ES12345678A, FR12345678901)
3. **Para facturas no comunitarias (NC)**: Solo se valida que el documento tenga al menos 2 caracteres
4. Corrige el NIF en el Excel y vuelve a cargar

---

### ⚠️ Advertencia: "Hay facturas con NIFs incorrectos. ¿Deseas continuar?"

**Causa**: Se detectaron errores de validación de NIF antes de enviar.

**Solución**:
- Puedes continuar si los NIFs son correctos pero el formato no es estándar
- O cancela, corrige los NIFs en el Excel, y vuelve a cargar
...
La aplicación incluye un **asistente de rectificativas** que facilita la creación de facturas rectificativas:

1. **Abrir el asistente**: Selecciona una fila en la tabla de facturas y haz clic en **"🧾 Rectificativa asistida"**
2. **Detección automática**: El asistente compara automáticamente la factura actual con la original y sugiere:
   - **Tipo de rectificativa** (R1, R4, etc.)
   - **Motivo** (código del 01 al 22 según normativa VeriFactu)
   - **Modalidad** (I - Por diferencias, S - Por sustitución)
3. **Detección de cambios**: El sistema detecta automáticamente si han cambiado:
   - Importes (base imponible, IVA, total)
   - Datos del cliente (NIF, nombre, dirección)
   - Datos del emisor
   - Conceptos de la factura
4. **Rectificativas de años anteriores**: Si intentas rectificar una factura de un año diferente al del Excel actual, el sistema te pedirá que cargues el Excel del año correspondiente
5. **Confirmación**: Revisa las sugerencias, ajusta si es necesario, y confirma para aplicar la rectificativa

La aplicación también detecta rectificativas automáticamente cuando:
- El número de factura ya empieza por `R1…R5`
- O cuando has configurado una rectificativa mediante el asistente

#### Resumen de escenarios habituales

| ¿Qué ha pasado? | ¿Qué pongo en el Excel? | ¿Qué marco en el asistente? |
| --- | --- | --- |
| Corrijo un error de IVA o cambio de tipo | Importes ya corregidos en positivo | `Tipo rectificativa: R1` · `Modalidad: I (Diferencias)` |
| Corrijo datos formales (nº, nombre, serie) pero los importes están bien | Importes sin cambios (positivos) | `Tipo rectificativa: R4` · `Modalidad: I (Diferencias)` |
| Anulo completamente una factura para sustituirla por otra | 1) Fila con importes **negativos** (anulación)<br>2) Fila nueva con la factura correcta | Para la fila negativa: `Modalidad: S (Sustitución)` y el tipo R1/R4 según el motivo |
| Concurso de acreedores / crédito incobrable | Importes en positivo según el nuevo devengo | `Tipo rectificativa: R2` (Art. 80.3) o `R3` (Art. 80.4)` · `Modalidad: I` |
| Ticket simplificado a corregir | Importes en positivo con los valores finales | `Tipo rectificativa: R5` · `Modalidad` según proceda |

#### Campos obligatorios en el asistente

1. **Factura original**: número exacto de la factura que corriges  
2. **Ejercicio y fecha original**: los de la factura inicial  
3. **Tipo rectificativa (VeriFactu)**: R1…R5 según la tabla anterior  
4. **Modalidad**:
   - `I · Rectificación por diferencias`: ajustes parciales  
   - `S · Rectificación por sustitución`: anulaciones completas (+ nueva factura)
5. **Motivo oficial (código)**: selecciona el que exige la AEAT (Ej: `16 - Base imponible`)
6. **Justificación opcional**: texto que se añadirá a `texto_libre` para documentar el cambio

El asistente guarda los valores en `responses/rectificativas_overrides.json` y se aplican automáticamente al generar el XML. Si vuelves a cargar el Excel, las opciones permanecen.
...
3. El cambio se aplica inmediatamente a toda la interfaz, incluyendo:
   - Tablas y tarjetas
   - Botones y controles
...
El nombre del archivo incluye el número de factura asignado por Facturantia (si está disponible), el cliente y el importe:
```
[Número Facturantia] - [Cliente] - [Importe].pdf
```
