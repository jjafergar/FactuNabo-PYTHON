...
def _norm_invoice_id(x: object) -> str:
    s = str(x).strip()
    # Si empieza por "R" seguido de dígitos, quitar el prefijo "R" para obtener el número original
    # Esto asegura que siempre usemos el número original para crear claves de override
    if s.upper().startswith("R") and len(s) > 1:
        s = s[1:]  # Quitar el prefijo "R"
...
def _rectificativa_key(invoice: object, empresa: object) -> str:
    inv = _norm_invoice_id(invoice)
    # Normalizar el nombre de la empresa: quitar tildes y convertir a minúsculas
    # Esto asegura que coincida con la normalización usada en main.py
    emp_raw = str(empresa or "").strip()
    emp = quitar_tildes_empresa(emp_raw).lower()
    return f"{inv}|{emp}"
...
def excel_date_to_datetime(excel_date):
    """
    Convierte una fecha de Excel a datetime, validando que esté en un rango razonable.
    Evita errores de 'timestamp too large' al compilar a .exe.
    """
    def _safe_now():
        """Obtiene la fecha actual de forma segura para .exe"""
        try:
            # Usar datetime estándar de Python y crear string, luego convertir a pd.Timestamp
            # para evitar problemas de timestamp en .exe compilado
            try:
                now = datetime.now()
                # Crear string de fecha usando strftime (más seguro que acceder a .year, .month, .day)
                date_str = now.strftime("%Y-%m-%d")
                return pd.Timestamp(date_str)
            except (OverflowError, OSError, ValueError):
                # Si falla, usar fecha fija segura
                return pd.Timestamp("2024-01-01")
        except (OverflowError, OSError, ValueError) as e:
            # Si hay error, usar fecha fija segura
            log(f"⚠️ Error al obtener fecha actual: {e}. Usando fecha por defecto.")
            try:
                return pd.Timestamp("2024-01-01")
            except Exception:
                # Si incluso esto falla, usar datetime y convertir
                return pd.Timestamp(datetime(2024, 1, 1))
        except Exception as e:
            # Fallback a una fecha fija segura si hay problemas
            log(f"⚠️ Error inesperado al obtener fecha actual: {e}. Usando fecha por defecto.")
            try:
                return pd.Timestamp("2024-01-01")
            except Exception:
                return pd.Timestamp(datetime(2024, 1, 1))
    ...
    try:
        # Si ya es un pd.Timestamp, validar y retornar directamente (evitar conversiones innecesarias)
        if isinstance(excel_date, pd.Timestamp):
            try:
                # Validar usando string para evitar acceder a atributos que puedan causar overflow
                fecha_str = str(excel_date)
                if len(fecha_str) >= 10:
                    # Extraer año del string
                    if fecha_str[4] == '-' and fecha_str[7] == '-':
                        year = int(fecha_str[:4])
                        if 1900 <= year <= 2100:
                            # Fecha válida, retornar directamente
                            return excel_date
                # Si no podemos validar desde string, usar fecha por defecto
                log(f"⚠️ No se pudo validar pd.Timestamp: {excel_date}. Usando fecha por defecto.")
                return _safe_now()
            except (ValueError, OverflowError, OSError, AttributeError) as e:
                log(f"⚠️ Error al validar pd.Timestamp: {excel_date}, error: {e}. Usando fecha por defecto.")
                return _safe_now()
        
        if isinstance(excel_date, (int, float)):
            # Validar que el número de días sea razonable (entre 0 y ~100000 días desde 1899-12-30)
            # Esto cubre aproximadamente desde 1899 hasta 2173
            if excel_date < 0 or excel_date > 100000:
                log(f"⚠️ Valor de fecha Excel fuera de rango: {excel_date}. Usando fecha por defecto.")
                return _safe_now()
            
            try:
                # Usar datetime de Python estándar para calcular la fecha (más seguro en .exe)
                from datetime import timedelta
                base_datetime = datetime(1899, 12, 30)
                result_datetime = base_datetime + timedelta(days=int(excel_date))
                
                # Validar que la fecha esté en rango razonable (1900-2100)
                # Usar strftime para obtener el año sin acceder directamente a .year
                try:
                    date_str_check = result_datetime.strftime("%Y-%m-%d")
                    year = int(date_str_check[:4])
                    if year < 1900 or year > 2100:
                        log(f"⚠️ Fecha resultante fuera de rango válido: {date_str_check}. Usando fecha por defecto.")
                        return _safe_now()
                except (OverflowError, OSError, ValueError, AttributeError):
                    log(f"⚠️ Error al validar rango de fecha. Usando fecha por defecto.")
                    return _safe_now()
                
                # Convertir a pd.Timestamp solo después de validar
                # Usar string para crear el timestamp de forma segura
                date_str = result_datetime.strftime("%Y-%m-%d")
                try:
                    result = pd.Timestamp(date_str)
                except (OverflowError, OSError, ValueError) as e:
                    log(f"⚠️ Error al crear pd.Timestamp desde {date_str}: {e}. Usando fecha por defecto.")
                    return _safe_now()
                
            except (OverflowError, OSError, ValueError, TypeError) as e:
                log(f"⚠️ Error al calcular fecha desde Excel: {excel_date}, error: {e}. Usando fecha por defecto.")
                return _safe_now()
            
            return result
        
        # Si es string u otro tipo, intentar parsear primero con datetime estándar
        try:
            # Intentar parsear con datetime estándar primero (más seguro)
            if isinstance(excel_date, str):
                excel_date_clean = str(excel_date).strip()
                # Intentar diferentes formatos comunes
                for fmt in ["%Y-%m-%d", "%d/%m/%Y", "%Y/%m/%d", "%d-%m-%Y"]:
                    try:
                        dt = datetime.strptime(excel_date_clean[:10], fmt)
                        # Validar rango ANTES de crear pd.Timestamp usando strftime
                        try:
                            date_str = dt.strftime("%Y-%m-%d")
                            year = int(date_str[:4])
                            if not (1900 <= year <= 2100):
                                continue  # Año fuera de rango, intentar siguiente formato
                            ...
                            try:
                                ...
                                if pd.isna(result):
                                    raise ValueError("Timestamp resultante es NaT")
                                return result
                            except (OverflowError, OSError, ValueError) as e:
                                # Si falla, usar fecha por defecto
                                log(f"⚠️ Error al crear timestamp desde {date_str}: {e}. Usando fecha por defecto.")
                                return _safe_now()
                        except (OverflowError, OSError, ValueError, AttributeError):
                            continue  # Error al formatear, intentar siguiente formato
                    except (ValueError, OverflowError, OSError):
                        continue
                
                # Si no funciona con datetime, intentar con pd.to_datetime pero con validación previa
                # Primero intentar extraer año del string para validar
                try:
                    # Buscar patrón de año (4 dígitos)
                    import re
                    year_match = re.search(r'\b(19\d{2}|20\d{2})\b', excel_date_clean)
                    if year_match:
                        year = int(year_match.group(1))
                        if year < 1900 or year > 2100:
                            log(f"⚠️ Año fuera de rango en fecha: {excel_date_clean}. Usando fecha por defecto.")
                            return _safe_now()
                except Exception:
                    pass  # Si no podemos validar, continuar con pd.to_datetime
                
                # Intentar parsear con pd.to_datetime pero con manejo de errores robusto
                try:
                    result = pd.to_datetime(excel_date, errors="coerce")
                    # Validar inmediatamente después de la conversión
                    if pd.isna(result):
                        log(f"⚠️ No se pudo convertir fecha: {excel_date_clean}. Usando fecha por defecto.")
                        return _safe_now()
                    # Validar rango de años antes de continuar
                    result_str = str(result)[:10]
                    if len(result_str) >= 10:
                        year = int(result_str[:4])
                        if year < 1900 or year > 2100:
                            log(f"⚠️ Fecha resultante fuera de rango: {result}. Usando fecha por defecto.")
                            return _safe_now()
                except (OverflowError, OSError, ValueError) as e:
                    log(f"⚠️ Error al convertir fecha con pd.to_datetime: {excel_date_clean}, error: {e}. Usando fecha por defecto.")
                    return _safe_now()
            else:
                # Para otros tipos (no string), validar antes de convertir
                try:
                    # Si es numérico, validar que no sea un timestamp Unix demasiado grande
                    if isinstance(excel_date, (int, float)):
                        ...
                        if excel_date > 1e10:
                            ...
                            excel_date = excel_date / 1000.0
                        # Validar que el timestamp esté en rango razonable (1970-2100)
                        if excel_date > 4102444800:  # 2100-01-01 en timestamp Unix
                            log(f"⚠️ Timestamp fuera de rango: {excel_date}. Usando fecha por defecto.")
                            return _safe_now()
                    
                    result = pd.to_datetime(excel_date, errors="coerce")
                    # Validar inmediatamente
                    if pd.isna(result):
                        log(f"⚠️ No se pudo convertir fecha: {excel_date}. Usando fecha por defecto.")
                        return _safe_now()
                except (OverflowError, OSError, ValueError) as e:
                    log(f"⚠️ Error al convertir fecha con pd.to_datetime: {excel_date}, error: {e}. Usando fecha por defecto.")
                    return _safe_now()
        ...
        # Validar que el resultado no sea NaT y esté en rango válido
        if pd.isna(result):
            log(f"⚠️ No se pudo convertir fecha: {excel_date}. Usando fecha por defecto.")
            return _safe_now()
        
        # Validar rango usando string para evitar problemas de timestamp
        try:
            result_str = str(result)[:10]  # Obtener solo la parte de fecha
            if len(result_str) >= 10:
                year = int(result_str[:4])
                if year < 1900 or year > 2100:
                    log(f"⚠️ Fecha resultante fuera de rango válido: {result}. Usando fecha por defecto.")
                    return _safe_now()
        except (ValueError, OverflowError, OSError) as e:
            log(f"⚠️ Error al validar rango de fecha: {e}. Usando fecha por defecto.")
            return _safe_now()
        
        return result
        
    except (OverflowError, OSError, ValueError) as e:
        # Capturar específicamente errores de timestamp fuera de rango
        log(f"⚠️ Error al convertir fecha {excel_date}: {e}. Usando fecha por defecto.")
        return _safe_now()
    except Exception as e:
        log(f"⚠️ Error inesperado al convertir fecha {excel_date}: {e}. Usando fecha por defecto.")
        return _safe_now()

def create_sub_element(parent, tag, text, default=None):
    if text is not None and pd.notna(text) and str(text).strip() != "":
        text_str = str(text)
        # Manejar pd.Timestamp y datetime de forma segura para .exe
        if isinstance(text, (datetime, pd.Timestamp)):
            try:
                # Para pd.Timestamp, NUNCA usar strftime() directamente - puede causar overflow
                # Siempre trabajar con la representación string
                if isinstance(text, pd.Timestamp):
                    # Estrategia: NUNCA acceder a métodos o atributos del timestamp
                    # Trabajar siempre con la representación string
                    text_repr = str(text)
                    
                    # La representación de pandas suele ser "YYYY-MM-DD HH:MM:SS" o "YYYY-MM-DD"
                    # Extraer solo la parte de fecha (primeros 10 caracteres)
                    if len(text_repr) >= 10 and text_repr[4] == '-' and text_repr[7] == '-':
                        text_str = text_repr[:10]
                        ...
                        if len(parts) == 3:
                            try:
                                ...
                                if 1900 <= year <= 2100 and 1 <= month <= 12 and 1 <= day <= 31:
                                    # Fecha válida, usar directamente
                                    pass
                                else:
                                    raise ValueError(f"Fecha fuera de rango: {text_str}")
                            except (ValueError, TypeError):
                                raise ValueError(f"Fecha inválida: {text_str}")
                        else:
                            raise ValueError(f"Formato de fecha inválido: {text_str}")
                    else:
                        # Si no tiene formato de fecha, usar fecha por defecto
                        log(f"⚠️ No se pudo extraer fecha de timestamp: {text_repr}. Usando fecha por defecto.")
                        text_str = "2024-01-01"
                else:
                    # Para datetime estándar de Python, usar strftime es seguro
                    try:
                        text_str = text.strftime("%Y-%m-%d")
                    except (OverflowError, OSError, ValueError) as e:
                        log(f"⚠️ Error al formatear datetime: {e}. Usando fecha por defecto.")
                        text_str = "2024-01-01"
            except (OverflowError, OSError, ValueError) as e:
                log(f"⚠️ Error al formatear fecha {text}: {e}. Usando representación string.")
                text_repr = str(text)
...
def validate_xml_against_xsd(xml_bytes: bytes, use_cache: bool = True) -> None:
    ...
    # Validación real con manejo robusto de errores de timestamp
    try:
        schema = _get_schema()
        # Intentar validar, pero capturar errores de timestamp específicamente
        try:
            # Primero parsear el XML para verificar que es válido sintácticamente
            import xml.etree.ElementTree as ET
            try:
                root = ET.fromstring(xml_bytes)
            except Exception as parse_err:
                raise ValueError(f"Error al parsear XML: {parse_err}")
            
            # Validar el XML parseado
            schema.validate(root)
        except (OverflowError, OSError) as timestamp_err:
            # Si es un error de timestamp, puede ser un problema interno de xmlschema
            # Verificar que el XML es parseable y las fechas tienen formato correcto
            try:
                import xml.etree.ElementTree as ET
                root = ET.fromstring(xml_bytes)
                
                # Verificar que las fechas tienen formato correcto (YYYY-MM-DD)
                fecha_elems = root.findall(".//fecha_emision") + root.findall(".//fecha_vencimiento")
                for fecha_elem in fecha_elems:
                    if fecha_elem.text:
                        fecha_text = fecha_elem.text.strip()
                        # Verificar formato YYYY-MM-DD
                        if len(fecha_text) == 10 and fecha_text[4] == '-' and fecha_text[7] == '-':
                            try:
                                year = int(fecha_text[:4])
                                if 1900 <= year <= 2100:
                                    # Fecha válida, continuar
                                    pass
                                else:
                                    raise ValueError(f"Fecha fuera de rango: {fecha_text}")
                            except ValueError:
                                raise
                        else:
                            raise ValueError(f"Formato de fecha inválido: {fecha_text}")
                
                # Si llegamos aquí, el XML es válido sintácticamente y las fechas están bien
                # El error de timestamp es interno de xmlschema, así que asumimos que el XML es válido
                log(f"⚠️ Advertencia: Error de timestamp interno en xmlschema durante validación XSD: {timestamp_err}")
                log(f"⚠️ El XML es parseable y las fechas tienen formato correcto. Continuando...")
                if use_cache:
                    xml_hash = _get_xml_hash(xml_bytes)
                    _VALIDATION_CACHE[xml_hash] = (True, None)  # Marcar como válido
                return
            except Exception as parse_err:
                # Si no podemos parsear el XML o las fechas son inválidas, entonces sí hay un error real
                raise ValueError(f"Error al validar XML: {parse_err}")
...
def create_xml_from_data(
    df_factura_single,
    df_conceptos_single,
    df_forma_pago_single,
    df_conceptos_texto_single=None,
    df_factura_all=None,
    df_conceptos_all=None,
    rectificativas_overrides=None,
):
    root = ET.Element("proformas")
    overrides_map = rectificativas_overrides or {}
    for _, factura_row in df_factura_single.iterrows():
        factura_row = factura_row.copy()
        proforma = ET.SubElement(root, "proforma")
        num_factura_actual = factura_row["NumFactura"]
        empresa_actual = factura_row["empresa_emisora"]
        key_override = _rectificativa_key(num_factura_actual, empresa_actual)
        override = overrides_map.get(key_override)
        # Debug: verificar si el override se está obteniendo correctamente
        if override:
            log(f"🔍 Override encontrado para {num_factura_actual} ({empresa_actual}): tipo_factura={override.get('tipo_factura')}, keys={list(override.keys())}")
        elif overrides_map:
            log(f"⚠️ Override NO encontrado para {num_factura_actual} ({empresa_actual}). Clave buscada: '{key_override}'. Claves disponibles: {list(overrides_map.keys())[:5]}")
        ...
        # Cabecera
        base_external_id = _norm_invoice_id(factura_row["NumFactura"])
        ...
        if override and override.get("tipo_factura") in allowed_tipos:
            tipo_factura_norm = override["tipo_factura"]
        # Política: rectificativa = R4 por defecto, salvo R1 cuando el motivo sea error fundado en derecho (cambio en tipo de IVA).
        # R1 solo para errores fundados en derecho (Art. 80 Uno, Dos y Seis LIVA), no para cambios simples en importes.
        if tipo_factura_norm.startswith("R") and not override:
            textos = " ".join([
        ...
        if override:
            justificacion = str(override.get("justificacion", "") or "").strip()
            if justificacion:
                existing = str(factura_row.get("texto_libre", "") or "")
                combinado = (existing + ("\n" if existing and justificacion else "") + justificacion).strip()
                factura_row["texto_libre"] = combinado[:2500]

        # Determinar external_id DESPUÉS de saber si es rectificativa
        override_external = None
        if override and "__external_id" in override:
            override_external = str(override["__external_id"]).strip()
        
        # Si es rectificativa y no hay external_id en el override, generar uno único
        if tipo_factura_norm.startswith("R") and not override_external:
            desired_external = f"R{base_external_id}"
            # Si excede 60 caracteres recortamos por la izquierda
            if len(desired_external) > 60:
                desired_external = desired_external[-60:]
            override_external = desired_external
            if override is not None:
                override["__external_id"] = override_external
        
        external_value = override_external or base_external_id
        if override is not None and "__external_id" not in override:
            override["__external_id"] = external_value
        
        # Serie (derivada; fuerza serie de rectificativas cuando aplique)
        def _infer_default_series(row: pd.Series) -> str:
            existing = (row.get("serie_factura") or "").strip()
            if existing:
                return existing
            nf_upper = str(
                row.get("NumFactura")
            ...
            if nf_upper.startswith("INT25_"):
                return "Int25_"
            if nf_upper.startswith("A25"):
                return "A25"
            if nf_upper.startswith("AB25"):
                return "AB25"
            return "25"

        serie_opt = ""
        if override:
            serie_opt = str(override.get("serie_factura", "") or "").strip()
        if not serie_opt:
            serie_opt = _infer_default_series(factura_row)

        if tipo_factura_norm.startswith("R"):
            rect_series_candidates = [
                str(override.get("serie_rectificativa") or "").strip() if override else "",
                str(override.get("serie_factura_rect") or "").strip() if override else "",
                str(factura_row.get("serie_rectificativa") or "").strip(),
                str(factura_row.get("serie_factura_rect") or "").strip(),
                os.environ.get("FACTUNABO_SERIE_RECT_DEFAULT", "").strip(),
            ]
            rect_series = next((val for val in rect_series_candidates if val), "")
            if not rect_series:
                rect_prefix = os.environ.get("FACTUNABO_SERIE_RECT_PREFIX", "R").strip()
                rect_base = serie_opt or os.environ.get("FACTUNABO_SERIE_BASE_DEFAULT", "25").strip() or "25"
                if rect_prefix and rect_base.upper().startswith(rect_prefix.upper()):
                    rect_series = rect_base
                elif rect_base.isdigit() and rect_prefix:
                    rect_series = f"{rect_prefix}{rect_base}"
                elif rect_prefix and rect_base:
                    rect_series = f"{rect_prefix}{rect_base}"
                else:
                    rect_series = rect_base or "R25"
            serie_opt = rect_series

        serie_opt = serie_opt or "25"
        factura_row["serie_factura"] = serie_opt

        # Crear elementos en el orden correcto según el XSD
        # 1. external_id (opcional)
        create_sub_element(proforma, "external_id", external_value)
        # 2. serie_factura (obligatorio)
        create_sub_element(proforma, "serie_factura", serie_opt)
        # 3. ejercicio (obligatorio)
        create_sub_element(proforma, "ejercicio", factura_row["ejercicio"], default="2025")
        # 4. fecha_emision (obligatorio)
        create_sub_element(proforma, "fecha_emision", excel_date_to_datetime(factura_row["fecha_emision"]))
        # 5. fecha_vencimiento (obligatorio)
        ...
        # 7. descripcion_general (obligatorio)
        create_sub_element(proforma, "descripcion_general", _build_descripcion_general(factura_row, df_conc))
        # 8. tipo_factura (obligatorio)
        create_sub_element(proforma, "tipo_factura", tipo_factura_norm, default="F1")
        # 9-10. factura_simplificada_art_* (opcionales, omitidos si no aplican)
        # 11-15. Campos de rectificativa (opcionales, solo si es rectificativa)
        if tipo_factura_norm.startswith("R"):
            create_sub_element(proforma, "factura_rectificativa", "1")
            num_rect = None
            if override and override.get("factura_rectificativa_numero"):
                num_rect = override.get("factura_rectificativa_numero")
            elif pd.notna(factura_row.get("factura_rectificativa_numero", None)):
                num_rect = factura_row.get("factura_rectificativa_numero")
            if num_rect:
                create_sub_element(proforma, "factura_rectificativa_numero", num_rect)

            ejercicio_rect = None
            if override and override.get("factura_rectificativa_ejercicio"):
                ejercicio_rect = override.get("factura_rectificativa_ejercicio")
            elif pd.notna(factura_row.get("factura_rectificativa_ejercicio", None)):
                ejercicio_rect = factura_row.get("factura_rectificativa_ejercicio")
            if ejercicio_rect:
                create_sub_element(proforma, "factura_rectificativa_ejercicio", ejercicio_rect)

            fecha_rect = None
            if override and override.get("factura_rectificativa_fecha_emision"):
                fecha_rect = override.get("factura_rectificativa_fecha_emision")
            elif pd.notna(factura_row.get("factura_rectificativa_fecha_emision", None)):
                fecha_rect = factura_row.get("factura_rectificativa_fecha_emision")
            if fecha_rect:
                create_sub_element(proforma, "factura_rectificativa_fecha_emision", fecha_rect)

            # Actualizado según Facturantia: el campo correcto es factura_rectificativa_motivo_codigo
            motivo_codigo = None
            if override and override.get("factura_rectificativa_codigo"):
                motivo_codigo = override.get("factura_rectificativa_codigo")
            else:
            ...
            if pd.notna(motivo_codigo):
                create_sub_element(proforma, "factura_rectificativa_motivo_codigo", motivo_codigo)
            
            rect_tipo = ""
            if override and override.get("factura_rectificativa_tipo"):
                rect_tipo = str(override.get("factura_rectificativa_tipo") or "").strip().upper()
            else:
                rect_tipo = str(factura_row.get("factura_rectificativa_tipo","") or "").strip().upper()
        ...
        # Validar campos obligatorios del cliente (provincia, poblacion, domicilio, cp)
        provincia_val = str(factura_row.get("cliente_provincia", "") or "").strip()
        if provincia_val:
            provincia_val = provincia_val[:20]
        else:
            raise ValueError(f"Campo obligatorio 'cliente_provincia' vacío para cliente {factura_row.get('cliente_numero_documento', 'N/A')}")
        
        poblacion_val = str(factura_row.get("cliente_poblacion", "") or "").strip()
        if not poblacion_val:
            raise ValueError(f"Campo obligatorio 'cliente_poblacion' vacío para cliente {factura_row.get('cliente_numero_documento', 'N/A')}")
        
        domicilio_val = str(factura_row.get("cliente_domicilio", "") or "").strip()
        if not domicilio_val:
            raise ValueError(f"Campo obligatorio 'cliente_domicilio' vacío para cliente {factura_row.get('cliente_numero_documento', 'N/A')}")
        
        cp_val = str(factura_row.get("cliente_cp", "") or "").strip()
        if not cp_val:
            raise ValueError(f"Campo obligatorio 'cliente_cp' vacío para cliente {factura_row.get('cliente_numero_documento', 'N/A')}")
        
        create_sub_element(cliente, "provincia", provincia_val)
        create_sub_element(cliente, "poblacion", poblacion_val[:50])
        create_sub_element(cliente, "domicilio", domicilio_val[:50])
        create_sub_element(cliente, "domicilio_2", factura_row.get("cliente_domicilio_2",""), default="")
        create_sub_element(cliente, "cp", cp_val[:5])
        ...
        # permitir_email_y_afc (después del cliente, según XSD)
        ...
        # Literales legales (al final, según XSD)
        ...
        # texto_libre, plantilla_facturas_emitidas, plantilla_facturas_proforma (al final, según XSD)
...
def send_proforma(xml_content, api_key, external_id, empresa, ejercicio, cliente_numero_documento,
                  api_email=None, api_url=None, api_timeout=None, use_offline_queue=False):
    """Envía la proforma a la API de Facturantia con manejo robusto de duplicados y errores de fecha."""
    import urllib.parse
    
    # Obtener timeout: primero del parámetro, luego de QSettings, luego por defecto 100
    # Validar que el timeout esté en un rango seguro (1-300 segundos) para evitar errores de PyTime_t
    if api_timeout is None:
        try:
            ...
            timeout_str = settings.value("api/timeout", "100")
            try:
                # Convertir a float primero para validar
                api_timeout = float(timeout_str)
                # Validar que el valor sea razonable (no más de 1000 segundos o milisegundos)
                # Si es muy grande, probablemente es un error de lectura, usar valor por defecto
                if api_timeout > 1000 or api_timeout < 0:
                    log(f"⚠️ Timeout inválido leído de configuración: {timeout_str}. Usando 100 segundos por defecto.")
                    api_timeout = 100
                else:
                    # Si el valor está entre 1 y 1000, podría estar en milisegundos si es > 100
                    # Pero si es <= 100, asumimos que ya está en segundos
                    if api_timeout > 100:
                        api_timeout = api_timeout / 1000.0
        ...
        try:
            ...
            if api_timeout > 1000 or api_timeout < 0:
                log(f"⚠️ Timeout inválido: {api_timeout}. Usando 100 segundos por defecto.")
                api_timeout = 100
            elif api_timeout > 100:
                # Si está entre 100 y 1000, podría estar en milisegundos
                api_timeout = api_timeout / 1000.0
            api_timeout = int(api_timeout)
        except (ValueError, TypeError):
            api_timeout = 100
    
    # Validar que el timeout esté en un rango seguro (1-300 segundos)
    # Esto previene errores de "timestamp too large to convert to C PyTime_t"
    if api_timeout < 1:
        api_timeout = 1
    elif api_timeout > 300:
        api_timeout = 300
    ...
    try:
        # Asegurar que el timeout sea un número válido antes de pasarlo a requests
        # Usar una tupla (connect_timeout, read_timeout) para mayor control
        # Esto previene errores de "timestamp too large to convert to C PyTime_t"
        connect_timeout = min(api_timeout, 30)  # Máximo 30 segundos para conectar
        read_timeout = api_timeout  # El resto para leer la respuesta
        timeout_tuple = (connect_timeout, read_timeout)
        
        resp = requests.post(url, data=xml_content, headers=headers, timeout=timeout_tuple)
        try:
...
def main(df_factura_historico=None, df_conceptos_historico=None, rectificativas_overrides=None):
    ...
    try:
        ...
        if rectificativas_overrides is None:
        ...
        for _, frow in df_factura.iterrows():
            ...
            key_override = _rectificativa_key(num, empresa)
            override_entry = rectificativas_overrides.get(key_override) if rectificativas_overrides else None
            tipo_override = ""
            if override_entry and override_entry.get("tipo_factura"):
                tipo_override = str(override_entry.get("tipo_factura") or "").upper()
            tipo_row = str(frow.get("tipo_factura", "") or "").upper()
            effective_tipo = tipo_override or tipo_row

            base_external_id = _norm_invoice_id(num)
            external_id_to_send = base_external_id
            is_rectificativa = effective_tipo.startswith("R")
            if is_rectificativa:
                if override_entry is None:
                    override_entry = rectificativas_overrides.setdefault(key_override, {})
                desired_external = f"R{base_external_id}"
                stored_external = str(override_entry.get("__external_id", "")).strip()
                if stored_external:
                    external_id_to_send = stored_external
                else:
                    # Intentar usar R{numero}; si excede 60 caracteres recortamos por la izquierda
                    if len(desired_external) > 60:
                        desired_external = desired_external[-60:]
                    override_entry["__external_id"] = desired_external
                    log(f"ℹ️ External ID rectificativa asignado para {base_external_id}: {desired_external}")
                    external_id_to_send = desired_external
            elif override_entry and override_entry.get("__external_id"):
                external_id_to_send = str(override_entry["__external_id"])
            ...
            if not api_key:
                ...
                summary_data.append({
                    "id": num,
                    "empresa": empresa,
                    "status": "ERROR_SIN_API_KEY",
                    "details": "Falta API Key",
                    "pdf_url": None,
                    "cliente": frow.get("cliente_nombre",""),
                    "importe": importe_total,
                    "external_id": external_id_to_send,
                    "num_original": base_external_id,
                })
                continue
            ...
            try:
                ...
                try:
                    xml_bytes = create_xml_from_data(
                        ...
                        rectificativas_overrides=rectificativas_overrides,
                    )
                except (OverflowError, OSError) as timestamp_err:
                    log(f"❌ ERROR DE TIMESTAMP en create_xml_from_data: {timestamp_err}")
                    log(f"❌ Tipo de error: {type(timestamp_err).__name__}")
                    import traceback
                ...
                try:
                    ...
                    # IDs sin ".0"
                    num_str = str(external_id_to_send)
                    try:
                        fnum = float(num_str)
                        if fnum.is_integer():
                ...
                try:
                    ...
                    log(f"❌ ERROR DE TIMESTAMP en validacion XSD: {timestamp_err}")
                    log(f"❌ Tipo de error: {type(timestamp_err).__name__}")
                    import traceback
                    log(f"❌ Traceback completo:\n{traceback.format_exc()}")
                    summary_data.append({
                        ...
                        "details": f"Error de timestamp en validación XSD: {timestamp_err}",
                        "pdf_url": None,
                        "cliente": frow.get("cliente_nombre",""),
                        "importe": importe_total
                    })
                    ...
                    log(f"❌ ERROR en validacion XSD: {xsd_err}")
                    log(f"❌ Tipo de error: {type(xsd_err).__name__}")
                    import traceback
                    log(f"❌ Traceback completo:\n{traceback.format_exc()}")
                    summary_data.append({
                        "id": num,
                ...
                try:
                    # Usar un timeout seguro y validado
                    safe_timeout = 5
                    if safe_timeout < 1:
                        safe_timeout = 1
                    elif safe_timeout > 300:
                        safe_timeout = 300
                    requests.get("https://www.facturantia.com", timeout=safe_timeout)
                    has_connection = True
                except:
                    has_connection = False
                ...
                # Obtener api_email y api_url del DataFrame si están disponibles
                api_email_from_df = frow.get("api_email", None)
                api_url_from_df = frow.get("api_url", None)
                try:
                    result = send_proforma(xml_bytes, api_key, external_id_to_send, empresa, ejercicio, cliente_doc,
                                         api_email=api_email_from_df, api_url=api_url_from_df, api_timeout=None,
                                         use_offline_queue=(use_offline and not has_connection))
                except (OverflowError, OSError) as timestamp_err:
                    log(f"❌ ERROR DE TIMESTAMP en send_proforma: {timestamp_err}")
                    log(f"❌ Tipo de error: {type(timestamp_err).__name__}")
                    import traceback
                    log(f"❌ Traceback completo:\n{traceback.format_exc()}")
                    raise
                ...
                if isinstance(details_json, dict):
                    ...
                    numeros_facturas = details_json.get("numeros_facturas", {})
                    if not isinstance(numeros_facturas, dict):
                        numeros_facturas = {}
                assigned_number = numeros_facturas.get(external_id_to_send)
                if procs:
                    for pr in procs:
                        ...
                        numero_factura_api = pr.get("numero_factura") or numeros_facturas.get(pr.get("external_id"))
                        if not numero_factura_api:
                            numero_factura_api = assigned_number
                        ...
                        summary_data.append({
                            ...
                            "external_id": pr.get("external_id", external_id_to_send),
                            "num_original": base_external_id,
                            "numero_asignado": numero_factura_api,
                        })
                else:
                    # [NUEVO] Obtener enlace QR si está disponible
                    enlace_qr = None
                    ...
                    summary_data.append({
                        ...
                        "external_id": external_id_to_send,
                        "num_original": base_external_id,
                        "numero_asignado": assigned_number,
                    })
            except (OverflowError, OSError) as timestamp_err:
                log(f"❌ ERROR DE TIMESTAMP capturado en bloque principal: {timestamp_err}")
                log(f"❌ Tipo de error: {type(timestamp_err).__name__}")
                import traceback
                log(f"❌ Traceback completo:\n{traceback.format_exc()}")
                summary_data.append({
                    ...
                    "details": f"Error de timestamp: {timestamp_err}",
                    "pdf_url": None,
                    "cliente": frow.get("cliente_nombre",""),
                    "importe": importe_total,
                    "external_id": external_id_to_send,
                ...
                log(f"❌ ERROR GENERAL capturado: {e}")
