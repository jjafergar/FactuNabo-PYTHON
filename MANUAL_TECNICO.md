...
**Propósito**: Módulo de validación de documentos de identificación españoles y europeos.

**Contiene**:
- `validate_nif(nif: str)`: Validación de NIF español
- `validate_cif(cif: str)`: Validación de CIF español
- `validate_nie(nie: str)`: Validación de NIE español
- `validate_nif_iva(nif_iva: str)`: Validación de NIF-IVA para países de la UE
- `validate_documento(documento: str, tipo: Optional[str])`: Auto-detección y validación
- `clean_documento(documento: str)`: Limpieza de documentos (espacios, guiones, etc.)

**Cuándo modificar**: Para añadir nuevos tipos de validación, modificar algoritmos de validación, o soportar nuevos países.
...
- **Validación de NIF/CIF/NIE**: Validación automática de documentos en la tabla de facturas
- **Asistente de rectificativas mejorado**: 
  - Detección automática de cambios entre factura original y actual
  - Sugerencias inteligentes de tipo de rectificativa (R1, R4)
  - Soporte para rectificativas de años anteriores
  - Carga automática de Excel histórico cuando es necesario
- **Tabla de previsualización mejorada**: 
  - Columna de concepto (primer concepto de cada factura)
  - Reordenamiento de columnas (Factura, Fecha, Empresa, Cliente, Concepto, Total)
  - Columna Fecha centrada y compacta
- **Tabla de resultados mejorada**: 
  - Columna de concepto
  - Uso de número asignado por Facturantia
- **Mejoras en modo oscuro**: 
  - Drop zone adaptado al tema
  - Estilos dinámicos según tema actual

**Nuevos métodos**:
- `validate_excel()`: Validación de NIFs en el Excel cargado
- `_detect_invoice_year()`: Detección del año de una factura
- `_detect_current_excel_year()`: Detección del año del Excel actual
- `_load_historical_excel_for_rectificativa()`: Carga de Excel histórico para rectificativas
- `_get_dropzone_styles()`: Generación de estilos del drop zone según tema
- `_update_dropzone_styles()`: Actualización de estilos del drop zone

---

### Mejoras en `prueba.py`

**Nuevas funcionalidades**:
- **Validación de campos obligatorios del cliente**: Validación explícita de `provincia`, `poblacion`, `domicilio`, `cp` antes de generar XML
- **Mejora en generación de external_id para rectificativas**: Generación automática de `R{original_id}` para evitar duplicados
- **Uso de número asignado por Facturantia**: Priorización de `numero_asignado` en respuestas de la API

---

### Mejoras en `macro_adapter.py`

**Nuevas funcionalidades**:
- **Primer concepto en mayúsculas**: Conversión automática a mayúsculas del primer concepto (con o sin importe)
- **Manejo mejorado de hojas auxiliares**: Filtrado de hojas no relacionadas con facturas (ej: "Tarifas", "Análisis")
- **Manejo de encabezados duplicados**: Función `_unique_headers()` para evitar errores en concatenación

---

### Mejoras en `app/ui/dialogs.py`

**Nuevas funcionalidades en `RectificativaDialog`**:
- **Detección de años diferentes**: Advertencia cuando se intenta rectificar una factura de otro año
- **Carga de Excel histórico**: Botón para cargar el Excel del año correspondiente directamente desde el diálogo
- **Actualización dinámica**: Actualización de sugerencias después de cargar Excel histórico
